...
│ └── pom.xml
│
├── audit-core/              # Library modules — parented by the reactor root,
...
| [`audit-core`](audit-core/README.md) ([ru](audit-core/README.ru.md)) | The platform's single audit envelope, sink SPI and redaction package, replacing the nine mechanisms nine modules had each invented: one `AuditEvent` with actor, resource, outcome and already-redacted attributes; one `AuditSink` whose failure policy is configuration rather than a `catch` block, so an observational event is logged and dropped while a state mutation fails the operation it describes; four composable sensitivity classifiers - by key name, by provenance, by declaration, by configured list - feeding one `Redactor` that covers scalars, attribute maps, header multimaps, structurally-walked JSON and form-encoded bodies behind one non-configurable mask; and one `ActorResolver` so that a row's `created_by` and the event describing that change name the same person by construction. Depends on nothing else in this repository, which is what lets the modules below `db-core` audit at all |
| [`audit-spring-boot-starter`](audit-spring-boot-starter/README.md) ([ru](audit-spring-boot-starter/README.ru.md)) | Everything the trail needs that a database, a scheduler or an outbox can provide: the append-only `audit_event` table with immutability enforced by `db-core`'s snapshot listener rather than by convention, a JPA sink that writes in the caller's own transaction so a change and its record commit together, an outbox sink that ships to a SIEM exactly once with the event id as the idempotency key, a per-category retention purge on `job-core` that measures its defaults in years and audits its own deletions, a QueryDSL read side deliberately unreachable from the write side, and Liquibase changesets that migrate `user_setting_audit` and `sync_audit_record` - rows included - into it |
...
| [`idempotency-spring-boot-starter`](idempotency-spring-boot-starter/README.md) ([ru](idempotency-spring-boot-starter/README.ru.md)) | The platform's one work-dedup store, promoted out of `notification-service`, where it was the last unfilled platform gap: claiming a caller-supplied key in a scope so the same ask cannot be acted on twice. One conditional upsert whose `ON CONFLICT ... DO UPDATE` makes the loser of a race block on the winner's row lock and read the committed winner - not `DO NOTHING`, which returns no row and leaves a `READ COMMITTED` re-select able to miss an uncommitted one, and not read-then-insert, which passes every single-threaded test and aborts a transaction that has already written real work. Two explicit claim modes, because one cannot serve both callers: `TRANSACTIONAL` commits with the caller's own work so a rollback frees the key, and `STANDALONE` runs an `IN_PROGRESS -> COMPLETED | FAILED` state machine under a lease, because a key stuck in progress behind a dead holder is a caller who can never retry. On top of it: an HTTP filter that **replays the original response** rather than re-running the handler - which is what a retried timed-out `POST` actually needs, instead of a flag saying "you already did this" - request fingerprinting so a recycled key is refused with a 422 instead of being answered with somebody else's resource, a Kafka record filter that claims inside the listener's transaction so at-least-once delivery stops meaning at-least-once effect, a batched retention purge on `job-core`'s leased lock, and a Liquibase changeset that moves `notification_idempotency`'s rows across because starting the new table empty means sending every in-flight notification twice during the deploy that introduces the module |
