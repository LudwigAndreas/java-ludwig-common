 ...
 * consuming application's own {@code spring.liquibase.change-log}, exactly as the outbox module does.
 *
...
@ConditionalOnProperty(prefix = "ludwig.job-core.liquibase", name = "enabled", matchIfMissing = true)
@AutoConfigureAfter(LiquibaseAutoConfiguration.class)
@EnableConfigurationProperties(JobCoreProperties.class)
public class JobCoreLiquibaseAutoConfiguration {

    /**
     * Applies {@code db/changelog/job-core/job-core-changelog.xml}.
     *
     * @param dataSource the application's data source
     * @return the module's Liquibase runner
     */
    @Bean(name = "jobCoreLiquibase")
    @ConditionalOnMissingBean(name = "jobCoreLiquibase")
    public SpringLiquibase jobCoreLiquibase(DataSource dataSource) {
